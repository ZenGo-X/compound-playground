...
import { symbolToAddress } from './client';
...
async function getCTokenAPR(
  ...
  // console.log('Supply rate', actualSupplyRate);
...
export async function getContractAPR(token: string): Promise<string> {
  if (token === 'ceth') {
    ...
    let contractAddress = symbolToAddress(token);
    return getCTokenAPR(CTOKEN_JSON_INTERFACE, contractAddress);
  }
  return 'Unknown token';
}
